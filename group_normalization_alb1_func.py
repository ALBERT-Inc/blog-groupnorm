...
def _call_fwd_reduce_cuda_kernel(x, eps):
    batch_size, groups, n_channels_per_group, n_elems_per_channel = x.shape

    mu = cuda.cupy.empty((batch_size, groups, 1, 1), dtype=x.dtype)
    inv_std = cuda.cupy.empty((batch_size, groups, 1, 1), dtype=x.dtype)
...
class GroupNormalization(function_node.FunctionNode):
    ...
    def forward_gpu(self, inputs):
        ...
        mu, inv_std = _call_fwd_reduce_cuda_kernel(x, self.eps)

        x_hat, y = cuda.elementwise(
            'T x, T mu, T inv_std, T gamma, T beta',
            'T x_hat, T y',
            'x_hat = (x - mu) * inv_std, '
            'y = (x - mu) * inv_std * gamma + beta',
            'gn_fwd_norm')(x, mu, inv_std, gamma, beta)
...
def _call_bwd_reduce_cuda_kernel(gy, x_hat, gamma):
    batch_size, groups, n_channels_per_group, n_elems_per_channel = gy.shape  # NOQA

    gy_and_gy_x_hat_sum_per_ch = cuda.cupy.empty(
        (2, batch_size, groups, n_channels_per_group, 1), dtype=gy.dtype)
    gygam_avg = cuda.cupy.empty(
        (batch_size, groups, 1, 1), dtype=gy.dtype)
    gygam_x_hat_avg = cuda.cupy.empty(
        (batch_size, groups, 1, 1), dtype=gy.dtype)
...
class GroupNormalizationGrad(function_node.FunctionNode):
    ...
    def forward_gpu(self, inputs):
        ...
        gy_and_gy_x_hat_sum_per_ch, gygam_avg, gygam_x_hat_avg = \
            _call_bwd_reduce_cuda_kernel(gy, x_hat, gamma)
        # Reuse x_hat for efficient memory access.
        cuda.elementwise(
            'T gy, T gamma, T x_hat, T inv_std, '
            'T gygam_avg, T gygam_x_hat_avg',
            '',
            'x_hat = inv_std * '
            '        (gy * gamma - gygam_avg - x_hat * gygam_x_hat_avg)',
            'gn_bwd_norm')(gy, gamma, x_hat, inv_std,
                           gygam_avg, gygam_x_hat_avg)
        gx = x_hat
